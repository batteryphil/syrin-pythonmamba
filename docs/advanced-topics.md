...
- **Rate limit thresholds:** When a rate limit threshold is hit, the threshold’s `action(ctx)` callback is run; implement desired behavior there (e.g. raise, wait, switch model). See [Rate Limiting](rate-limiting.md).
